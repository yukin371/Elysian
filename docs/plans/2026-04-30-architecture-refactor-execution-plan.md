...
> - `6C-3 / OpenAPI 并行消费试点` 已启动最小落地：`apps/example-vue/src/lib/platform-api/generated-types.ts` 已从生成产物提取 `auth / customer / settings / tenants / users / roles / notifications / posts / departments / menus / dictionary types` 所需响应与请求体别名，并覆盖当前 `dictionary items` 列表响应形状；对应 `platform-api` 子模块已切到并行消费这些类型；当前仍不修改 `lib/platform-api/types.ts` 的 canonical owner，后续只在更多低风险模块试点稳定后再评估是否切换前端类型网关
...
| 6C-3 切换类型网关导入源 | `lib/platform-api/types.ts` | 已启动并行消费试点：`auth.ts`、`customer.ts`、`settings.ts`、`tenants.ts`、`users.ts`、`roles.ts`、`notifications.ts`、`posts.ts`、`departments.ts`、`menus.ts`、`dictionaries.ts` 已先经由 `generated-types.ts` 消费 OpenAPI 生成类型；`types.ts` 仍保持 schema 类型网关 canonical owner，待更多低风险模块试点稳定后再评估是否切换 |
