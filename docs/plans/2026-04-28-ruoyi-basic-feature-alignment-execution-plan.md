...
本轮后续执行可以让界面工作流先暴露问题，但不能让界面绕过契约直接定义能力。

适用范围：

- `WP-1` 已有工作区闭环收口应优先从 `apps/example-vue` 的真实使用路径切入。
- 对用户可见的后台日常体验，先列出页面验收路径，再回查现有 API、权限点、错误语义和数据 owner 是否足够支撑。
- 若现有契约已满足，优先只改前端装配与交互状态；若契约不足，再按 canonical owner 做最小后端或 persistence 补口。

禁止扩展：

- 不为了界面效果新增第二套路由框架、第二套 API client owner 或跨模块 shared helper。
- 不把 mock-only 页面、静态占位或规划态按钮写成“已完成能力”。
- 不因为界面上出现某个入口，就默认导入、统计、预览、批量治理等重能力已经进入当前主线。
...
- 权限不足、空态、加载态与失败态统一收口
- 从真实后台使用路径反推缺口，但每个新增动作都必须有对应 API 契约、权限语义或明确的当前不可用状态
...
- `apps/server`：`/system/posts` 列表 / 详情 / 导出 / 创建 / 更新 / 状态动作
...
3. 通知已补当前筛选批量已读，文件已补当前筛选批量删除；租户 / operation-log 的后台日常交互继续打磨
