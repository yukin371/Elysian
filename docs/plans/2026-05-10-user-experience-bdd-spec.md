...
本 BDD 默认服务以下判断，不再把“体验优化”理解成只补页面文案或装饰细节：

1. `Parity`：竞品已普遍具备、用户高频感知的后台能力，必须至少达到可用下限。
2. `Better`：竞品也具备，但 Elysian 需要把路径、反馈、证据和交接做得更顺。
3. `Unique`：Elysian 必须形成选择理由的能力，例如结构化生成链路、可审查产物和治理证据。

因此，后续实现如果只能回答“这页又多了一个功能点”，但不能回答“用户为什么更愿意继续用 Elysian”，就不应视为高优先级体验优化。
...
- GVA：后台 starter、系统模块、代码生成和菜单/权限接线效率，重点参考“做一个新模块时路径是否短、接线是否顺”。
- elysia-admin：同生态后台能力与模块化组织方式，重点参考“在 `Elysia + Bun` 语境下，用户是否会认为 Elysian 更完整、更顺手、更可持续交付”。
...
6. 用户不会因为“我们也有这个功能”而选择 Elysian；页面必须同时服务能力下限、路径更短和选择理由这三件事。
