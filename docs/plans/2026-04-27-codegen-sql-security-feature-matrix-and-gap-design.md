...
> 优先级说明（`2026-04-28`）：本文件继续作为 `codegen / SQL / security` 子轨道矩阵使用，不再代表仓库当前第一优先级。当前仓库级优先顺序已切到“若依基础功能对齐”，见 [2026-04-28-ruoyi-basic-feature-alignment-matrix.md](./2026-04-28-ruoyi-basic-feature-alignment-matrix.md)。
...
- preview / apply / detail 响应会返回目标目录 diff 与冲突解释
...
以下顺序只适用于 `codegen / SQL / security` 子轨道内部，不覆盖仓库当前总优先级。
