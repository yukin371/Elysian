...
- 下一步：若继续推进真实发布评审，则按 `Tenant 发布 blocker 确认单` 收集剩余 `8` 个真实 blocker 证据，不再额外处理 workflow 结构风险
...
1. 按已固定的 GitHub `Tenant Release Rehearsal` 默认入口推进 tenant 发布演练；仅在 GitHub 手动入口不可用时回退到 runbook + shell env 路径。
