...
> **状态**：P6A 已完成，P6B1/P6B2 已完成，P6B3 进行中
...
| `P6B3` 租户管理与治理 | 租户 CRUD + 初始化脚本 + 配置覆盖 + 升级路径 ADR | `P6B1` 完成 | 超级管理员可创建管理租户，一键初始化新租户，并具备升级/发布演练收尾链路 | 🚧 进行中 |
...
1. ✅ 已完成 `Phase 4` 收口（`P4D/P4E`），生成链路已进入“可回归、可门禁”状态。
2. ✅ 已完成 `Phase 5 / P5A`（`AI -> Schema`）归档，6 case 验收全绿，handoff / replay / acceptance / gate / index / finalize 链路稳定。
...
6. 🚧 正在推进 `P6B3`：租户管理、`tenant:init`、setting fallback、ADR-0009、真实 PostgreSQL 验证、CI、滚动观察、迁移/发布演练 runbook、checklist、样例与 GitHub manual rehearsal 已完成；GitHub `Tenant Release Rehearsal` 现已固定为默认 rehearsal 入口，剩余重点为 `Tenant 发布 blocker 确认单` 的 `8` 个真实 blocker 证据。
