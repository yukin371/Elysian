...
- `validate -> e2e-generator-(matrix|cli|studio|safe-apply) -> reports:index -> reports:gate`
...
| `e2e-generator-*.json` | generator matrix / cli / studio 报告 | `e2e:generator:*` |
...
- generator gate 失败：先看 `e2e-generator-reports-index.json`，按 `source=matrix|cli|studio` 回到对应入口
