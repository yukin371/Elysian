...
项目仍然处于早期开发阶段，欢迎讨论和贡献
## 项目定位

- 后端内核固定为 `Elysia`，承担 API、鉴权、流式 AI、OpenAPI、模块装配。
- 前端预计支持 `Vue` 和 `React` 两条路线，并提供uniapp模板
- UI 采用“协议层 + 官方预设层”设计，先提供企业预设与自建风格预设，再逐步开放第三方 Adapter。
- 预置企业项目中的高频重复能力，例如用户、角色、权限、菜单、部门、字典、日志、任务、文件、通知、审计、配置等。
- 提供代码生成器，输入结构化规格，输出模块骨架代码，覆盖后端、前端、文档、测试等多个维度，让开发者专注于业务实现。
- 提供 AI 辅助能力，生成结构化规格、补全重复逻辑、加速开发实现。
...
- `apps/example-vue`
  首个前端示例，采用 Vue + Vite + Tailwind，现已接入登录态、菜单树与权限驱动展示
- `packages/schema`
...
- `packages/ui-enterprise-vue`
  Vue 企业预设承载包，底座已固定为 `Arco Design Vue`，已转移到 `TDesign`
