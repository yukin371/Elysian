...
describe("buildGenerationPreviewReport", () => {
  it("builds a report with file preview and sql preview", async () => {
    ...
    expect(report.databaseChangePlan.operations[0]?.tableName).toBe("tickets")
    ...
    expect(report.sqlPreview.contents).toContain("CREATE TABLE tickets (")
  ...
  it("keeps standard CRUD preview report aligned with inject-based workspace templates", async () => {
    ...
    expect(report.sqlPreview.contents).toContain("CREATE TABLE posts (")
  })
})
