...
describe("writeModuleFiles", () => {
  ...
  it("treats identical existing files as no-op when conflict strategy is fail", async () => {
    const directory = await mkdtemp(join(tmpdir(), "elysian-generator-"))
  ...
  it("fails on changed existing files when conflict strategy is fail", async () => {
    const directory = await mkdtemp(join(tmpdir(), "elysian-generator-"))
    const existingPath = join(directory, "modules/customer/customer.schema.ts")

    await mkdir(dirname(existingPath), { recursive: true })
    await writeFile(existingPath, "export const custom = true\n", "utf8")
  ...
  it("overwrites generated files with overwrite-generated-only strategy", async () => {
    const directory = await mkdtemp(join(tmpdir(), "elysian-generator-"))
    const managedPath = join(directory, "modules/customer/customer.schema.ts")
    ...
    const schemaEntry = secondRun.find(
      (item) => item.path === "modules/customer/customer.schema.ts",
    )

    expect(schemaEntry?.written).toBe(true)
    expect(
      secondRun
        .filter((item) => item.path !== "modules/customer/customer.schema.ts")
        .every((item) => item.written === false),
    ).toBe(true)
  })
  ...
  it("supports concurrent overwrite-generated-only runs on managed files", async () => {
    ...
    expect(runA.every((item) => item.written === false)).toBe(true)
    expect(runB.every((item) => item.written === false)).toBe(true)
...
describe("applyGenerationPreviewReport", () => {
  ...
  it("applies an identical preview report as no-op with fail strategy", async () => {
    const directory = await mkdtemp(join(tmpdir(), "elysian-generator-"))
    ...
    expect(report.files.every((item) => item.plannedAction === "skip")).toBe(
      true,
    )
    expect(result.files.every((item) => item.written === false)).toBe(true)
  })
