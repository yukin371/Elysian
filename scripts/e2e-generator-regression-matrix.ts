...
const runCase = async (schemaName: string, frontendTarget: FrontendTarget) => {
  ...
  try {
    ...
    assert(
      overwriteRunA.every((item) => item.written === false),
      `[${schemaName}/${frontendTarget}] overwrite-generated-only should no-op when managed files already match`,
    )
    await assertManifestMatches(
      outputDir,
      schemaName,
    ...
    assert(
      overwriteRunB.every((item) => item.written === false),
      `[${schemaName}/${frontendTarget}] second overwrite-generated-only should no-op when files still match`,
    )
    ...
    await writeFile(
      join(outputDir, expectedPaths[0] ?? ""),
    ...
    assert(
      failAsExpected,
      `[${schemaName}/${frontendTarget}] fail strategy should reject changed existing files`,
    )
    ...
    assert(
      concurrentRunA.every((item) => item.written === false) &&
        concurrentRunB.every((item) => item.written === false),
      `[${schemaName}/${frontendTarget}] concurrent overwrite-generated-only should keep matching files as no-op`,
    )
